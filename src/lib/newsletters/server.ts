...
export function renderNewsletterHtml(opts: { edition:any; subscriber:any; siteUrl:string; logoUrl:string }) {
  ...
  const adLink = (ad:any) => `${siteUrl}/api/ad-click?id=${encodeURIComponent(String(ad.id || ""))}&placement=${encodeURIComponent(String(ad.placement || "newsletter"))}&source=newsletter&to=${encodeURIComponent(link(ad.destination_url || siteUrl))}`;
  const adHtml = (placement:string) => ads.filter((ad:any)=>ad.placement===placement).slice(0,1).map((ad:any)=>`<tr><td style="padding:14px 24px"><div style="padding:7px 0;font:700 10px Arial,sans-serif;letter-spacing:.12em;color:#666;text-transform:uppercase">Advertisement</div><a href="${escapeHtml(adLink(ad))}" style="display:block;text-decoration:none;color:#10243b">${ad.creative_url?`<img src="${escapeHtml(absoluteUrl(siteUrl,ad.creative_url))}" alt="${escapeHtml(ad.alt_text||ad.advertiser_name)}" width="672" style="display:block;width:100%;height:auto;border:0">`:`<div style="border:1px solid #d8d2c8;padding:18px;font:700 18px Georgia,serif">${escapeHtml(ad.advertiser_name)}</div>`}</a></td></tr>`).join("");
  const poll = content.poll;
  const pollHtml = content.include_poll && poll ? `<tr><td style="padding:0 24px 24px"><div style="border:1px solid #b8b0a3;padding:18px;background:#f4f0e8"><div style="font:700 10px Arial,sans-serif;color:#1e5f94;letter-spacing:.13em;text-transform:uppercase">Community Pulse</div><h3 style="margin:7px 0 12px;font:700 23px/1.2 Georgia,serif;color:#101820">${escapeHtml(poll.question)}</h3>${(poll.poll_options||[]).slice(0,4).map((option:any)=>`<a href="${escapeHtml(link('/community-pulse'))}" style="display:block;margin-top:7px;padding:9px 11px;border:1px solid #cfc8bb;background:#fff;color:#10243b;font:700 13px Arial,sans-serif;text-decoration:none">${escapeHtml(option.label)} <span style="float:right">Vote →</span></a>`).join("")}<p style="margin:13px 0 0;font:12px Arial,sans-serif"><a href="${escapeHtml(link('/community-pulse'))}" style="color:#1e5f94;font-weight:bold">VIEW THE POLL AND RESULTS →</a></p></div></td></tr>` : "";
  const utilityHtml = `${content.include_weather_tides?`<tr><td style="padding:0 24px 16px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse"><tr><td style="padding:15px;border:1px solid #b8b0a3"><div style="font:700 10px Arial,sans-serif;color:#1e5f94;letter-spacing:.13em;text-transform:uppercase">Island conditions</div><div style="margin:5px 0 9px;font:700 20px Georgia,serif">Weather, marine conditions and tides</div><a href="${escapeHtml(link('/weather'))}" style="font:700 12px Arial,sans-serif;color:#1e5f94;text-decoration:none">WEATHER →</a> <span style="color:#aaa">|</span> <a href="${escapeHtml(link('/weather/tides'))}" style="font:700 12px Arial,sans-serif;color:#1e5f94;text-decoration:none">TIDES →</a></td></tr></table></td></tr>`:""}${content.include_marketplace_promo?`<tr><td style="padding:0 24px 24px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse"><tr><td style="padding:15px;border:1px solid #b8b0a3"><div style="font:700 10px Arial,sans-serif;color:#1e5f94;letter-spacing:.13em;text-transform:uppercase">Community marketplace</div><div style="margin:5px 0 9px;font:700 20px Georgia,serif">Buy, sell and find local services</div><a href="${escapeHtml(link('/marketplace'))}" style="font:700 12px Arial,sans-serif;color:#1e5f94;text-decoration:none">BROWSE THE MARKETPLACE →</a></td></tr></table></td></tr>`:""}`;
  const supportHtml = `<tr><td style="padding:0 24px 24px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse"><tr><td style="padding:18px;border-top:3px solid #101820;background:#f4f0e8"><div style="font:700 10px Arial,sans-serif;color:#1e5f94;letter-spacing:.13em;text-transform:uppercase">Community supported</div><div style="margin:5px 0 8px;font:700 21px Georgia,'Times New Roman',serif;color:#101820">Help keep Haida Gwaii News free for everyone.</div><p style="margin:0 0 12px;font:14px/1.45 Arial,sans-serif;color:#4b4b4b">Reader support helps sustain independent local reporting, island information and the print paper.</p><a href="${escapeHtml(link('/support'))}" style="font:700 12px Arial,sans-serif;color:#1e5f94;text-decoration:none">SUPPORT HGN →</a></td></tr></table></td></tr>`;
...
<table role="presentation" width="720" class="page" cellspacing="0" cellpadding="0" style="width:720px;max-width:720px;background:#fffdf8;border:1px solid #cfc8bb;border-collapse:collapse">
  ...
  ${storyRows}
  ${pollHtml}${utilityHtml}${adHtml("mid_banner")}${eventsHtml}${adHtml("business_card")}${supportHtml}
