...
export function newsflowToneClasses(tone: string) {
...
export function toneForNewsflowItem(item: Row) {
  const status = String(item.flow_status || "").toLowerCase();
...
export async function getNewsflowSnapshot() {
  const [items, checks] = await Promise.all([
    safeSelect("newsflow_items", "sort_order", true, 100),
    safeSelect("newsflow_checks", "sort_order", true, 50),
  ]);

  const open = items.filter((item) => !item.is_done && !["done", "dropped"].includes(String(item.flow_status || "").toLowerCase()));
  const urgent = items.filter((item) => String(item.priority || "").toLowerCase() === "urgent" || String(item.flow_status || "").toLowerCase() === "blocked");
  const active = items.filter((item) => String(item.flow_status || "").toLowerCase() === "active");
  ...
  if (active.length > 0) score += 8;
  if (open.length > 0 && open.length <= 5) score += 6;
  if (open.length > 8) score -= 12;
  if (urgent.length > 0) score -= 22;
  score = Math.max(0, Math.min(100, Math.round(score)));

  return { score, items, checks, open, urgent, active };
}
