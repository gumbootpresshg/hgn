...
export default function EditAdPage() {
  ...
  async function handleUpload(file: File | null) {
    ...
    try {
      ...
      if (!ad.alt_text) patch("alt_text", ad.title || "Advertisement")
  ...
  const selectedPlacement = placements.find((placement) => placement.placement_key === ad.placement_key)
  ...
  if (!ad) return <main className="px-6 py-10">Loading...</main>

  return (
    <main className="mx-auto max-w-5xl space-y-6 px-6 py-10">
      <Link href="/admin/ads" className="text-sm font-semibold text-slate-600">← Back to Ad Manager</Link>
      ...
      <section className="rounded-3xl border bg-white p-8 shadow-sm">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <label className="block">
            ...
            <select value={ad.placement_key || ""} onChange={(e) => patch("placement_key", e.target.value)} className="mt-2 w-full rounded-2xl border px-4 py-3">
              {placements.map((placement) => (
                <option key={placement.placement_key} value={placement.placement_key}>{placement.label} ({placement.placement_key})</option>
              ))}
            </select>
          </label>
        ...
        <label className="mt-4 block">
          ...
          <textarea value={ad.html_code || ""} onChange={(e) => patch("html_code", e.target.value)} rows={5} className="mt-2 w-full rounded-2xl border px-4 py-3" />
        </label>

        <label className="mt-4 flex items-center gap-2">
          <input type="checkbox" checked={Boolean(ad.is_house_ad)} onChange={(event) => patch("is_house_ad", event.target.checked ? 1 : 0)} />
          <span className="text-sm font-semibold">House ad / HGN self-promotion</span>
        </label>

        <label className="mt-4 block">
          <span className="text-sm font-semibold">Notes</span>
          <textarea value={ad.notes || ""} onChange={(e) => patch("notes", e.target.value)} rows={3} className="mt-2 w-full rounded-2xl border px-4 py-3" />
        </label>

        {ad.image_url ? (
          <div className="mt-6 overflow-hidden rounded-2xl border bg-slate-50 p-3">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={ad.image_url} alt={ad.alt_text || ad.title || "Ad preview"} className="mx-auto max-h-80 object-contain" />
          </div>
        ) : null}
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          <button disabled={saving} onClick={() => save("active")} className="rounded-full bg-emerald-600 px-4 py-2 text-xs font-bold uppercase tracking-wide text-white disabled:opacity-50">Activate</button>
          <button disabled={saving} onClick={() => save("paused")} className="rounded-full bg-slate-200 px-4 py-2 text-xs font-bold uppercase tracking-wide text-slate-700 disabled:opacity-50">Pause</button>
          <button disabled={saving} onClick={() => save("draft")} className="rounded-full bg-amber-500 px-4 py-2 text-xs font-bold uppercase tracking-wide text-white disabled:opacity-50">Draft</button>
        </div>
      </section>
    </main>
  )
