...
import { formatPublishingDate, type PublishingSettings } from "@/lib/publishing-settings"

export default function ArticleListCard({ article, fallbackLabel = "Article", publishingSettings }: { article: any; fallbackLabel?: string; publishingSettings: PublishingSettings }) {
    ...
    <Link href={`/articles/${article.slug}`} className="group block border-b border-stone-300 py-5 first:pt-0">
      <div className="grid gap-4 sm:grid-cols-[1fr_180px] sm:items-start">
        <div className="min-w-0">
          ...
          <p className="mt-3 text-xs text-stone-500">{articleAuthor(article)}{article.published_at ? ` · ${formatPublishingDate(article.published_at, publishingSettings)}` : ""}</p>
        </div>
