...
export default async function Home() {
  ...
  // Mobile is intentionally composed like a front page rather than stacking the entire
  // desktop news column before the sidebar. This keeps section variety near the top.
  const mobileLatest = chronological.filter((a) => a.slug !== main?.slug).slice(0, 4)
  const mobileUsed = new Set([main?.slug, ...mobileLatest.map((a) => a.slug), opinion?.slug].filter(Boolean))
  const mobileNews = chronological.filter((a) => isLocalNews(a) && !mobileUsed.has(a.slug)).slice(0, 4)
  mobileNews.forEach((a) => mobileUsed.add(a.slug))
  const mobileSports = chronological.filter((a) => isSports(a) && !mobileUsed.has(a.slug)).slice(0, 3)
  mobileSports.forEach((a) => mobileUsed.add(a.slug))
  const mobileColumns = chronological.filter((a) => isColumn(a) && !mobileUsed.has(a.slug)).slice(0, 3)
  mobileColumns.forEach((a) => mobileUsed.add(a.slug))
  const mobileMoreNews = chronological.filter((a) => isLocalNews(a) && !mobileUsed.has(a.slug)).slice(0, 6)
