...
export default function InviteSignup({ params }: { params: Promise<{ token: string }> }) {
  ...
  async function load(){
    const { data, error } = await supabase.from("contributor_invites").select("*").eq("token", token).eq("status", "pending").single();
