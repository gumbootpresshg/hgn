...
function StatusCard({ row }: { row: Row }) {
  ...
  return (
    <article className={`rounded-2xl border p-4 shadow-sm ${deploymentTone(status, row.severity)}`}>
      ...
      <p className="mt-2 text-sm leading-6 opacity-80">
        {row.notes || row.expected_result || row.note_body || "Being checked before HGN goes online for soft beta."}
      </p>
    </article>
  );
}

export default async function SoftBetaDeploymentStatusPage() {
  const snapshot = await getSoftBetaDeploymentSnapshot();
  const visible = [...snapshot.steps, ...snapshot.env, ...snapshot.smoke, ...snapshot.rollback].slice(0, 12);
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      <section className="rounded-3xl border bg-white p-6 shadow-sm lg:p-8">
        ...
        <h1 className="mt-2 text-4xl font-black text-hgnNavy">Soft Beta Deployment Status</h1>
        <p className="mt-3 max-w-3xl text-slate-700">
          A clean public-safe view of the final deployment checks before HGN goes online for admin/editor soft beta testing.
        </p>
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2">
        {visible.map((row, index) => (
          <StatusCard key={row.id || labelFor(row) || index} row={row} />
        ))}
      </section>
    </main>
  );
