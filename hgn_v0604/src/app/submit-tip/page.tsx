...
export default function SubmitTipPage() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (error) alert(error.message); else {
      await notifyPublicSubmission("story_tip", {
        title: String(f.get("title") || "Story tip"),
        submitterName: String(f.get("name") || ""),
        submitterEmail: String(f.get("email") || ""),
        publicAdminUrl: "/admin/submissions",
      });
      setDone(true);
    }
  }
