...
set business_name = coalesce(nullif(business_name, ''), label, placement_key, 'HGN House Ad'),
    label = coalesce(nullif(label, ''), placement_key, business_name, 'Ad Placement'),
    placement_key = coalesce(nullif(placement_key, ''), lower(regexp_replace(coalesce(label, business_name, id::text), '[^a-zA-Z0-9]+', '_', 'g'))),
...
insert into public.ad_placements (placement_key, label, business_name, description, page_area, max_ads, is_active)
select 'site_top', 'Site Top Banner', 'HGN House Ad', 'Top banner across the site.', 'global', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'site_top');

insert into public.ad_placements (placement_key, label, business_name, description, page_area, max_ads, is_active)
select 'home_middle', 'Home Middle Banner', 'HGN House Ad', 'Mid-page homepage banner.', 'home', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'home_middle');

insert into public.ad_placements (placement_key, label, business_name, description, page_area, max_ads, is_active)
select 'home_sidebar', 'Home Sidebar', 'HGN House Ad', 'Sidebar placement on homepage/news layout.', 'home', 3, true
where not exists (select 1 from public.ad_placements where placement_key = 'home_sidebar');

insert into public.ad_placements (placement_key, label, business_name, description, page_area, max_ads, is_active)
select 'article_top', 'Article Top', 'HGN House Ad', 'Ad near the top of articles.', 'article', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'article_top');

insert into public.ad_placements (placement_key, label, business_name, description, page_area, max_ads, is_active)
select 'article_bottom', 'Article Bottom', 'HGN House Ad', 'Single clean bottom ad under articles.', 'article', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'article_bottom');

insert into public.ad_placements (placement_key, label, business_name, description, page_area, max_ads, is_active)
select 'classifieds_top', 'Classifieds Top', 'HGN House Ad', 'Top of classifieds/marketplace.', 'classifieds', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'classifieds_top');

insert into public.ad_placements (placement_key, label, business_name, description, page_area, max_ads, is_active)
select 'real_estate_top', 'Real Estate Top', 'HGN House Ad', 'Top of real estate page.', 'real_estate', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'real_estate_top');

insert into public.ad_placements (placement_key, label, business_name, description, page_area, max_ads, is_active)
select 'events_top', 'Events Top', 'HGN House Ad', 'Top of events page.', 'events', 1, true
