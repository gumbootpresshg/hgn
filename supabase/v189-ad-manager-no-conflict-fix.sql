...
select 'site_top', 'Site Top Banner', 'Top banner across the site.', 'global', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'site_top');

insert into public.ad_placements (placement_key, label, description, page_area, max_ads, is_active)
select 'home_middle', 'Home Middle Banner', 'Mid-page homepage banner.', 'home', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'home_middle');

insert into public.ad_placements (placement_key, label, description, page_area, max_ads, is_active)
select 'home_sidebar', 'Home Sidebar', 'Sidebar placement on homepage/news layout.', 'home', 3, true
where not exists (select 1 from public.ad_placements where placement_key = 'home_sidebar');

insert into public.ad_placements (placement_key, label, description, page_area, max_ads, is_active)
select 'article_top', 'Article Top', 'Ad near the top of articles.', 'article', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'article_top');

insert into public.ad_placements (placement_key, label, description, page_area, max_ads, is_active)
select 'article_bottom', 'Article Bottom', 'Single clean bottom ad under articles.', 'article', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'article_bottom');

insert into public.ad_placements (placement_key, label, description, page_area, max_ads, is_active)
select 'classifieds_top', 'Classifieds Top', 'Top of classifieds/marketplace.', 'classifieds', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'classifieds_top');

insert into public.ad_placements (placement_key, label, description, page_area, max_ads, is_active)
select 'real_estate_top', 'Real Estate Top', 'Top of real estate page.', 'real_estate', 1, true
where not exists (select 1 from public.ad_placements where placement_key = 'real_estate_top');

insert into public.ad_placements (placement_key, label, description, page_area, max_ads, is_active)
select 'events_top', 'Events Top', 'Top of events page.', 'events', 1, true
