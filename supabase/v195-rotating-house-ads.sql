...
-- Adds example HGN house ads into the actual ad rotation system.
-- Safe to rerun.
...
-- House ad HTML uses inline styles so it renders inside existing AdSlot safely.
...
select
  ...
  1,
  true,
  'Example rotating house ad. Keep active to promote ad sales when paid inventory is light.',
  now(),
  now()
from (
  values
...
set status = coalesce(nullif(status, ''), 'draft'),
    ...
    is_house_ad = coalesce(is_house_ad, false),
    updated_at = now();

create index if not exists idx_ads_house_rotation
on public.ads (is_house_ad, placement_key, status, rotation_weight);
