...
import { getNextUpSnapshot, nextUpTone, toneForNextUp } from "@/lib/next-up";

export const dynamic = "force-dynamic";

type Row = Record<string, any>;

async function addNextUpItem(formData: FormData) {
  "use server";
...
async function updateNextUpStatus(formData: FormData) {
  "use server";
...
function NextUpCard({ item }: { item: Row }) {
  const tone = toneForNextUp(item);

  return (
    <article className={`rounded-2xl border p-5 shadow-sm ${nextUpTone(tone)}`}>
      ...
      <form action={updateNextUpStatus} className="mt-4 flex flex-wrap gap-2">
        <input type="hidden" name="id" value={item.id} />
        {["next", "ready", "waiting", "blocked", "published", "dropped"].map((status) => (
...
export default async function NextUpPage() {
  ...
  const stats = [
    ["Next-up score", `${snapshot.score}%`, "How clear the next publishing move is", scoreTone],
    ["Ready", snapshot.ready.length, "Can move now", snapshot.ready.length ? "good" : "warn"],
    ["Blocked", snapshot.blocked.length, "Needs a fix", snapshot.blocked.length ? "bad" : "good"],
    ["Frontpage", snapshot.frontpage.length, "Homepage-related items", snapshot.frontpage.length ? "blue" : "neutral"],
  ] as const;
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10">
      <div className="flex flex-col gap-5 border-b pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm font-black uppercase tracking-widest text-hgnBlue">v117 Next-Up Desk</p>
          <h1 className="mt-2 text-5xl font-black text-hgnNavy">Next-Up Desk</h1>
          <p className="mt-3 max-w-3xl text-slate-700">
            A tiny board for the admin/editor beta workflow: what should move next, what is ready, and what is blocking the next publish decision.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Link href="/next-up-status" className="hgn-btn-primary">Status</Link>
        </div>
      </div>

      <section className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {stats.map(([label, value, helper, tone]) => (
          <div key={label} className={`rounded-2xl border p-5 shadow-sm ${nextUpTone(String(tone))}`}>
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="grid gap-4">
          <h2 className="text-2xl font-black text-hgnNavy">What moves next</h2>
          {snapshot.items.length ? (
            snapshot.items.map((item) => <NextUpCard key={item.id} item={item} />)
          ) : (
            <div className="rounded-2xl border border-dashed bg-slate-50 p-6 text-sm text-slate-600">No next-up items yet.</div>
          )}
        </div>

        <div className="grid gap-6">
          <form action={addNextUpItem} className="hgn-card grid gap-3 p-6">
            <h2 className="text-2xl font-black text-hgnNavy">Add next-up item</h2>
            <input name="title" placeholder="What should move next?" className="rounded-xl border px-4 py-3" />
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <select name="urgency" className="rounded-xl border px-4 py-3">
          ...
          <section className="hgn-card p-6">
            <h2 className="text-2xl font-black text-hgnNavy">Quick checks</h2>
